...
# ---- clean stale binaries/assets (publish never deletes leftovers; a stale
#      *.Views.dll from an older publish causes 'view not found' errors) ----
if [ -d "$APP_DIR" ]; then
    log "Removing stale build output (keeping DB, uploads, App_Data)"
    find "$APP_DIR" -mindepth 1 -maxdepth 1 \
        ! -name 'aicigasesti.db' ! -name 'aicigasesti.db-shm' ! -name 'aicigasesti.db-wal' \
        ! -name 'App_Data' ! -name 'wwwroot' -exec rm -rf {} +
    # wwwroot is regenerated by publish; keep only user uploads
    [ -d "$APP_DIR/wwwroot" ] && find "$APP_DIR/wwwroot" -mindepth 1 -maxdepth 1 \
